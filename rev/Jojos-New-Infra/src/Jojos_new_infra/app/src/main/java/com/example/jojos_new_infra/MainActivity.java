...
public class MainActivity extends AppCompatActivity {
    ...
    */
    ...
    public void main(checkStuff checkStuff, ViewSwitcher vSwitch) {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView webView1, final WebResourceRequest request) {
                ...
                new Thread(() -> {
                    String query = String.valueOf(request.getUrl().getEncodedQuery());
                    List<String> params = Arrays.asList(query.split("="));
                    if (params.size() > 1 && !(query.equals("v=4.7.0"))) {
                        String pass = null;
                        try {
                            pass = URLDecoder.decode(params.get(params.size() - 1).split("&")[0], StandardCharsets.UTF_8.toString());
                        } catch (UnsupportedEncodingException e) {
                            e.printStackTrace();
                        }
                        String username = null;
                        try {
                            username = URLDecoder.decode(params.get(1).split("&")[0], StandardCharsets.UTF_8.toString());
                        } catch (UnsupportedEncodingException e) {
                            e.printStackTrace();
                        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ViewSwitcher vSwitch = findViewById(R.id.viewswitcher);
